...
async function fetchLoans() {
  ...
  const bookTitleFilter =
    document.getElementById("bookTitleFilter").value || "";

  let queryString = `?user=${encodeURIComponent(
    userFilter
  )}&book_title=${encodeURIComponent(bookTitleFilter)}`;
...
function displayLoans(loans) {
  ...
  loans.forEach((loan) => {
    ...
    loanRow.innerHTML = `
            <td>${loan.loan_id}</td>
            <td>${loan.user_id}</td>
            <td><a href="user_loans.html?user_id=${loan.user_id}">${
      loan.user_name || "Unknown"
    }</a></td>  <!-- Link to view user loans -->
